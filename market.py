...
class MainWindom:
    ...
    def buyOrder(self, x, y):
        ...
        #判断是否已挂单
        time.sleep(0.5)
        if ToolMgr.RGBEqual(1882,426,183,154,127):
            pass # 回到主界面
        #取卖单价
        time.sleep(0.5)
        SellPrice = ToolMgr.Read(1199, 676)
        #点买单
        ToolMgr.ClickAndRGB(1146, 584)
        #取买单价
        time.sleep(0.5)
        buyPrice = ToolMgr.Read(1199, 676)
        #算差价
        ...
        ToolMgr.Writer(buyPrice, 1199, 676)
        #点创建按钮
        ToolMgr.ClickAndRGB(1424, 775)
        #点确认提交按键
        ToolMgr.ClickAndRGB(1345, 588)

    #循环往复滑动界面
    def LoopScore(self):
        posRgbArr = ToolMgr.GetAreaRGB()
        ToolMgr.Scroll(-100)
        pyautogui.sleep(0.3)
